...
- `--provider azure`：Azure OpenAI 兼容的 `/openai/v1` 接口（如 **Kimi-K2.6** 部署）。Azure v1 API 与 OpenAI 同构、用 Bearer 鉴权，复用同一适配器；**部署名即模型名**（`--model Kimi-K2.6`），MiniMax 专属的 `thinking` 字段会自动忽略。
- `--provider poe`：Poe 的 OpenAI 兼容 API（`https://api.poe.com/v1`，Bearer 鉴权），默认模型 **gpt-5.4**（`--model gpt-5.4`，或设 `POE_API_KEY`/`POE_MODEL`）；同样复用 OpenAI 适配器，`thinking` 字段不发送。
...
接入 Poe（OpenAI 兼容 `https://api.poe.com/v1`，Bearer 鉴权，默认模型 `gpt-5.4`）：

```bash
export POE_API_KEY='<your-poe-api-key>'     # 也可用 OPENAI_API_KEY
# POE_MODEL 可选，默认 gpt-5.4；POE_BASE_URL 可选，默认 https://api.poe.com/v1

tcm-fuzzywiki build-llm \
  --input build/input/chapters.normalized.csv \
  --config configs/tcm_fuzzywiki.yaml \
  --output build/llm_wiki \
  --provider poe --model gpt-5.4 --workers 4
```
